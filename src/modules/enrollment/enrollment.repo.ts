...
export class EnrollmentRepository {
  ...
  async existsByStudentAndCourse(studentId: number, courseId: number): Promise<boolean> {
    try {
      const pool = getPool();
  ...
  async findAll(query: PaginationQuery): Promise<{ enrollments: any[]; total: number }> {
    try {
      const pool = getPool();
      const { page = 1, size = 10, search, studentId, courseId, teacherId, status } = query;

      let whereConditions: string[] = [];
      const request = pool.request();

      if (search) {
        whereConditions.push(`(s.fullName LIKE @search OR s.studentCode LIKE @search OR c.courseName LIKE @search OR c.courseCode LIKE @search)`);
      ...
      const dataResult = await request.query(`
        SELECT 
          e.enrollId, e.studentId, e.courseId, e.status, e.enrolledAt,
